...
var svg = d3.select("#republican-chart").append("svg")
...
var republicanGrandparent = svg.append("g")
...
var legend = d3.select("#republican-legend").append("svg")
...
function initialize(root) {
  ...
  // republicanTreemap layout, but not here because of our custom implementation.
  ...
  // Compute the republicanTreemap layout recursively such that each group of siblings
  ...
  function layout(d) {
    if (d._children) {
      // republicanTreemap nodes comes from the republicanTreemap set of functions as part of d3
      republicanTreemap.nodes({_children: d._children});
...
d3.json("./data/republican-treemap.json", function(root) {
  ...
  function display(d) {
    ...
    var g1 = svg.insert("g", ".republicanGrandparent")
        .datum(d)
        .attr("class", "depth");
